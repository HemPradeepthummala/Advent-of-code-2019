...
const isInclude = ([x, y], coords) => {
  return coords.some(([cx, cy]) => cx === x && cy === y);
};
...
const move = ([w1, w2]) => {
  ...
  for (let counter = 0; counter < big; counter++) {
    if (counter < w1.moves) {
      moveW1(wire1);
      wire1coords.push([wire1.x, wire1.y]);
    }
    if (counter < w2.moves) {
      moveW2(wire2);
      wire2coords.push([wire2.x, wire2.y]);
    }
  }
};
...
const res = wire1coords.filter(([x, y]) => isInclude([x, y], wire2coords));
console.log(res);
const result = res.map(([x, y]) => Math.abs(x) + Math.abs(y));
console.log(result);
...
console.log(wire1coords,wire2coords)

for (let i = 0; i < res.length; i++) {
  const minCords = res[i];
  const wire1index = wire1coords.findIndex(([x,y]) => x === minCords[0]&&minCords[1]===y);
  const wire2index = wire2coords.findIndex(([x,y]) => x === minCords[0]&&minCords[1]===y);
  console.log(wire1index, wire2index, 'indices');
  movements.push(wire1index + 1 + wire2index + 1);
}
